...
function solveVariations(a, b, c) {
    ...
    html += `<div class="step-explanation">f'(x) = ${formatNumber(2 * a)}x + ${formatNumber(b)} est une <strong>fonction affine</strong></div>`;
    html += '<div class="step-explanation">💡 Pour déterminer le signe d\'une fonction affine ax + b :</div>';
    html += '<div class="step-explanation">• On trouve la racine : x₀ où f\'(x) = 0</div>';
    html += '<div class="step-explanation">• <strong>Si a > 0</strong> : la fonction est négative avant x₀, puis positive après</div>';
    html += '<div class="step-explanation">• <strong>Si a < 0</strong> : la fonction est positive avant x₀, puis négative après</div>';
    html += `<div class="step-expression">Ici, le coefficient de x est ${formatNumber(2 * a)}, donc :</div>`;
    if (2 * a > 0) {
    ...
    // Tableau de signes et variations combiné
    const fX0 = a * x0 * x0 + b * x0 + c;
    html += '<div class="step">';
